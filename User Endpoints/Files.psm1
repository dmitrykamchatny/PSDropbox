...
.Synopsis
   Create a new file.
.DESCRIPTION
   Create a new file with contents provided in the request. Maximum 150MB for this call.

   Calls to this endpoint count as data transport calls for any Dropbox Business teams with limited number of data transport calls allowed per month.

   Refer to https://www.dropbox.com/developers/documentation/http/documentation#files-upload.
.EXAMPLE
   PS> Upload-DropboxFile -DropboxPath /Documents/test.txt -InputFile C:\test.txt -WriteMode add -Token <access token>

   Uploads file test.txt to Dropbox path /Documents/test.txt .
#>
function Upload-DropboxFile {
    [CmdletBinding()]
    Param(
        # Dropbox path to save file.
        [parameter(Mandatory)]
        [alias("DbxPath")]
        [string]$DropboxPath,
        ...
        [parameter(Mandatory)]
        [string]$InputFile,
        # Select action to take if file already exists.
        [parameter(Mandatory)]
        [ValidateSet("add","overwrite","update")]
        [string]$WriteMode,
        # File revision if update is specified.
        [string]$Revision,
        # Dropbox server will automatically rename file if filename conflict occurs.
        [switch]$AutoRename,
        # Suppress notifications.
        [switch]$Mute,
        # Be more strict on how each WriteMode detects conflicts.
        [switch]$StringConflict,
        # Dropbox API access token.
    ...
    Process {
        ...
        if (((Get-Item -Path $InputFile).Length / 1024 /1024 ) -lt $SizeLimit) {
            $Body = @{
                path=$DropboxPath
                autorename=$AutoRename.IsPresent
                mute=$Mute.IsPresent
                strict_conflict=$StringConflict.IsPresent
            }
            if ($WriteMode -eq "update") {
                $Body.Add("mode",@{".tag"="update";update=$Revision})
            } else {
                $Body.Add("mode",$WriteMode)
            }
...
.DESCRIPTION
   Refer to https://www.dropbox.com/developers/documentation/http/documentation#files-download.
.EXAMPLE
   PS> Download-DropboxFile -DropboxPath /test/test.txt -OutFile C:\test.txt -Token <access token>

   Download Dropbox file test.txt located in the test folder and output downloaded file to C:\.
#>
function Download-DropboxFile {
    ...
    Param(
        ...
        [string]$Id,
        # Dropbox file revision.
        [parameter(Mandatory,ParameterSetName="Revision")]
        [string]$Revision,
        # Path to output downloaded file.
        [parameter(Mandatory)]
        [string]$OutFile,
        # Dropbox API access token.
        [parameter(Mandatory,HelpMessage="Enter access token")]
